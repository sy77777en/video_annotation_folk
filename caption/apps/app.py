...
class VideoAnnotationApp:
    ...
    def _run_review_portal(self):
        ...
        # File check
        if "file_check_passed_review" not in st.session_state:
            if self.video_utils.file_check(st.session_state.video_urls, video_data_dict, self.data_manager):
                st.session_state.file_check_passed_review = True
            else:
                return
    ...
    def _render_review_sidebar(self, video_data_dict: dict):
        ...
        selected_config = st.selectbox(
            "Select a task:",
            config_names,
            index=config_names.index(st.session_state.get('last_config_id_review', config_names[0])),
            key="selected_task_review",
        )
        ...
        selected_video = st.selectbox(
            ...
            video_urls,
            format_func=self.video_utils.get_video_format_func(output_dir, video_urls, self.data_manager),
            index=video_urls.index(st.session_state.get('last_selected_video_review', video_urls[0])),
            key="selected_video_review"
        )
        ...
        preserved_keys = [
            'api_key', 'last_config_id_review', 'selected_config_review',
            'last_video_id_review', 'last_selected_video_review', 
            'file_check_passed_review', 'logged_in', 'video_urls', 'logged_in_user', 
    ...
    def _handle_config_change_review(self, selected_config: str, config_names: list):
        ...
        if 'last_config_id_review' not in st.session_state:
            st.session_state.last_config_id_review = selected_config
        elif st.session_state.last_config_id_review != selected_config:
            # Clear state on config change
            keys_to_remove = [key for key in st.session_state 
                             if key not in ['api_key', 'last_config_id_review', 'file_check_passed_review', 'logged_in', 
                                          'video_urls', 'last_video_id_review', 'last_selected_video_review', 'logged_in_user', 
                                          'personalized_output', 'selected_portal', 'login_method', 'target_annotator']]
            for key in keys_to_remove:
                del st.session_state[key]
            st.session_state.last_config_id_review = selected_config
        ...
        if 'last_video_id' not in st.session_state:
            ...
            keys_to_remove = [key for key in st.session_state 
                             if key not in ['api_key', 'last_config_id', 'selected_config', 'last_video_id', 
                                          ...
                                          'logged_in_user', 'personalized_output', 'selected_portal', 'login_method', 'target_annotator']]
            for key in keys_to_remove:
                del st.session_state[key]
            st.session_state.last_video_id = video_id
            st.session_state.last_selected_video = selected_video
    ...
    def _handle_video_change_review(self, video_id: str, selected_video: str):
        """Handle video changes for review"""
        if 'last_video_id_review' not in st.session_state:
            st.session_state.last_video_id_review = video_id
            st.session_state.last_selected_video_review = selected_video
        elif st.session_state.last_video_id_review != video_id:
            # Clear state on video change
            keys_to_remove = [key for key in st.session_state 
                             if key not in ['api_key', 'last_config_id_review', 'selected_config_review', 'last_video_id_review', 
                                          'last_selected_video_review', 'file_check_passed_review', 'logged_in', 'video_urls', 
                                          'logged_in_user', 'personalized_output', 'selected_portal', 'login_method', 'target_annotator']]
            for key in keys_to_remove:
                del st.session_state[key]
            st.session_state.last_video_id_review = video_id
